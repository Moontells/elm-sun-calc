...
import List exposing (append, filter, filterMap, map, range)
import SunCalc exposing (Coordinated, CrossResult)
...
previousNewMoon zone startTime =
    let
        iterate last_phase time =
            let
                new_phase =
                    SunCalc.moonIllumination time |> .phase
            in
            case last_phase of
                Nothing ->
                ...
                Just phase ->
                    if new_phase > phase then
                        binsearchNewMoon time (nextDay zone time)
...
binsearchNewMoon left right =
    let
        offsetToPosix =
            floor >> (+) (posixToMillis left) >> millisToPosix

        phase =
            offsetToPosix >> SunCalc.moonIllumination >> .phase
        ...
        startOffset =
            posixToMillis right - posixToMillis left |> toFloat
    in
    search 20 0 startOffset |> offsetToPosix



-- 20 iterations give subsecond precision
    ...
    let
        moons current =
            if before current start then
                ...
                current :: (moons <| previousNewMoon zone <| addHours -1 zone current)
    in
    moons end |> List.reverse
...
lunarDaysByPeriod zone coord intervalStart intervalEnd =
    let
        startOfLunarMonth =
            previousNewMoon zone intervalStart

        newMoons =
            newMoonsInInterval zone intervalStart intervalEnd

        altitude time offset =
            SunCalc.moonAltitude (addMinutes offset zone time) coord
...
baseAt posix cross =
    let
        base =
            Maybe.map (\offset -> TimeUtils.addFloatHours offset posix)
    in
...
insertNewMoons newmoons lunardays =
    let
        insertion time =
            Temporal (Interval (Just time) Nothing) { lunarMonthStart = True }

        iterate collected days moons =
            case ( days, moons ) of
                ( current :: future_days, newMoonTime :: future_moons ) ->
                    let
                        (Temporal (Interval dayStart _) _) =
                            current
                    in
                    case dayStart of
                        Nothing ->
                            iterate (current :: collected) future_days moons

                        Just dayStartTime ->
                            if before newMoonTime dayStartTime then
                                iterate (insertion newMoonTime :: collected) days future_moons

                            else
                                iterate (current :: collected) future_days moons

                ( _, [] ) ->
                    append (List.reverse days) collected

                ( [], _ ) ->
                    collected
    in
    iterate [] lunardays newmoons |> List.reverse
...
batch next collected =
    case collected of
        ...
        (Temporal current_interval additional) :: previous ->
            let
                (Temporal next_interval _) =
                    next
            in
            case ( current_interval, next_interval ) of
        ...
        numerate days index =
            case days of
                ...
                (Temporal interval dayinfo) :: remaining ->
                    let
                        daynumber =
                            if dayinfo.lunarMonthStart then
                                Just 1
                    ...
                    case daynumber of
                        Nothing ->
                            numerate remaining Nothing

                        Just n ->
                            Temporal interval { day = n } :: (numerate remaining <| Just (n + 1))
    in
    numerate lunardays Nothing
...
toMoonDay coord (Temporal (Interval start end) dayinfo) =
    ...
    , phase =
        if dayinfo.day == 1 then
            ...
            SunCalc.moonIllumination start |> .phase
    }
...
trimByInterval start end =
    let
        isInInterval moonday =
            before moonday.startTime end && before start moonday.endTime
    in
    filter isInInterval
...
lunarCalendar zone coord start end =
...
arrangeByDays zone start end moondays =
    dayIntervals zone start end
